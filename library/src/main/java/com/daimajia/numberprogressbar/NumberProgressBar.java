...
public class NumberProgressBar extends View {
    ...
    private void calculateDrawRectF() {
        float curProgress = getProgress(), maxProgress = getMax();

        mCurrentDrawText = String.format("%.2f", curProgress * 100 / maxProgress);
    ...
    public void setMax(int maxProgress) {
        if (maxProgress > 0) {
            this.mMaxProgress = maxProgress * 100;
            invalidate();
        }
    }
    ...
    public void setProgress(float progress) {
        if (progress <= getMax() && progress >= 0) {
            this.mCurrentProgress = (int) (progress * 100);
            invalidate();
        }
    }
